...
public class League {
	...
	private void parseSchedule(String leagueURL) {
		...
		try {
			...
			for (int i = 0; i < matchDays.length; i++) {
				 ...
				 */
				if (s.matches(Utensils.MATCHDAY_AND_DATE_TYPE1) // Kolejka 1 -
																// 21
																// paŸdziernika
						|| s.matches(Utensils.MATCHDAY_AND_DATE_TYPE2) // Kolejka
																		// 1 -
																		// 20-21
																		// paŸdziernika
						|| s.matches(Utensils.MATCHDAY_AND_DATE_TYPE3)) // Kolejka
																		// 1 -
																		// 31
																		// wrzeœnia-1
																		// paŸdziernika
				...
				/**
				 * Wyszukuje linijkê z nazwami klubów, wynikiem i dat¹, mecze
				 * rozegrane i nie rozegrane
				 */
				if (s.matches(".*-?.*"))// s.matches(Utensils.MECZ) ||
										// s.matches(Utensils.MECZ2))
				{
					boolean flag = false;
					...
					addDataAboutMatchesInMatchdays(matchDayNumber, matchDayDate, homeClubName, awayClubName, result,
							dateOfConcreteMatch, 0);
				}
	...
	private int howManyWordsInClubName(String[] tab) {
		...
		for (int i = 0; i < tab.length; i++) {
			if ((tab[i].matches(".*\\D+.*") || tab[i].matches("\\d{4}")) // np.
																			// ROW
																			// 1946
																			// Rybnik
					&& !tab[i].matches("\\d+-\\d+") // bramki strzelone-
													// stracone (12-11)
	...
	public void addDataAboutMatchesInMatchdays(String matchDayNumber, String matchDayDate, String homeClubName,
			String awayClubName, String result, String dateOfConcreteMatch, int attendance) {
		MatchDay matchday = new MatchDay(matchDayNumber, matchDayDate);
		Match match = new Match(matchDayNumber, homeClubName, awayClubName, result, null, null, null, null,
